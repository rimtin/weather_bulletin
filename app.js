...
async function fetchFirst(urls){
  for(const url of urls){
    try{ const r=await fetch(url,{cache:"no-cache"}); if(!r.ok) continue;
      const j=await r.json(); console.log("[Map] Loaded:",url); return j; }catch{}
  }
  throw new Error("No GeoJSON found");
}

/* ---------- colored classification table ---------- */
function buildCloudTable(){
  const table=document.getElementById("cloudTable"); if(!table) return;
...
async function drawMap(svgId){
  ...
  paths
    ...
    .on("pointermove", function(event,d){
      ...
      if(x+ttW>vw) x=vw-ttW-pad; if(y+ttH>vh) y=vh-ttH-pad;
  ...
  paths.each(function(d){
    const key=norm(String(d.properties?.[MATCH_KEY]??"")); if(!key) return;
  ...
  if(svgId==="#indiaMapDay2"){
    ...
    updateMapColors(); // will also color the selects
  }
}

/* ---------- table with merged State cells ---------- */
...
function highlight(label,on){
  ...
  ["#indiaMapDay1","#indiaMapDay2"].forEach(svgId=>{
    const nodes=indexByGroup[svgId]?.get(key); if(!nodes) return;
    nodes.forEach(n=>{
